...
export default function AllProjects() {
  ...
  return (
    <section className="mt-20 mb-[-15px] w-full py-10 px-6 relative mt-20 bg-gradient-to-b from-gray-900 to-gray-800 rounded-3xl text-white overflow-hidden shadow-2xl">
      ...
      {allProjects.technology.map((technology) => (
        <motion.div
          key={technology.id}
          className="mt-10"
          initial={{ opacity: 0 }}
        ...
        >
          <motion.h2
            className="text-[30px] md:text-[35px] lg:text-[45px] mt-6 text-gray-100 font-extrabold border-t pt-3 md:text-left text-center"
          >
            {technology.name}
          </motion.h2>
